...
var server = http.createServer(function(request, response){
  ...
  if(urlParsed.pathname === '/' || urlParsed.pathname === '/index'){
    ...
    if(typeof(urlParsed.query) == 'object' && urlParsed.query !== null){
      ...
      collection.findOne(
        {_id: ObjectID(urlParsed.query['id'])}, projection, function(error, data){
          if(error) throw error;
    ...
    if(urlParsed.pathname === '/edit'){
      ...
      collection.findOne(
        ...
        if(difference >= 0){
          ...
          collection.update(
            {_id: doc['_id']}, {$set:{'happybonus.points': difference}}, function(error){
            if(error) throw error;

            collection.findOne(
              {_id: ObjectID(data['secondId'])}, projection, function(error, doc2){
